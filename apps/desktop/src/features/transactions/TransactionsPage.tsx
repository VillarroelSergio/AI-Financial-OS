      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="premium-card rounded-lg p-xl space-y-lg">
          ...
          <div className="grid grid-cols-2 gap-lg">
            ...
            <label className="space-y-xs"><span className="text-caption text-stone">Cuenta</span><select required className="w-full bg-white/[.035] border border-hairline-dark rounded-lg px-md py-sm text-body-sm text-on-dark" value={form.account_id} onChange={(e) => setForm((f) => ({ ...f, account_id: e.target.value }))}><option value="">Seleccionar cuenta</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></label>
