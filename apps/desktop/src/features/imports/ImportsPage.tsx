      ...
      {!preview && (
        <section className="premium-card rounded-lg p-7">
          ...
          <div className="mb-6 max-w-sm">
            ...
            <select value={accountId} onChange={(e) => setAccountId(e.target.value)} className="w-full rounded-lg border border-hairline-dark bg-white/[.035] px-3 py-2.5 text-sm text-on-dark">
          ...
          <label className={`min-h-40 rounded-lg border border-dashed flex flex-col items-center justify-center bg-black/20 ${accountId ? "border-hairline-dark hover:border-primary cursor-pointer" : "border-hairline-dark opacity-50 cursor-not-allowed"}`}>
            ...
            <span className="mt-3 flex flex-wrap gap-2 justify-center">{KNOWN_FORMATS.map((f) => <span key={f} className="rounded-full border border-hairline-dark bg-white/[.03] px-2.5 py-1 text-[11px] text-stone">{f}</span>)}</span>
          ...
          <section className="premium-card rounded-lg overflow-hidden">
            <div className="p-5 flex flex-wrap items-center gap-3 border-b border-hairline-dark">
              ...
              <div className="ml-auto flex gap-2 text-xs">
                ...
                {preview.rows_skipped > 0 && <button onClick={() => toggleFilter("skipped")} className={chipClass("skipped", "bg-white/[.06] text-stone")}>{preview.rows_skipped} omitidas</button>}
